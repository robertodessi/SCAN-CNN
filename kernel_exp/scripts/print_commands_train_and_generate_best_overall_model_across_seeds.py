...
def main():
    ...
    train_list = []
    for split in splits:
        for kernel_enc in ['1', '2', '3', '4', '5']:
            for kernel_dec in ['1', '2', '3', '4', '5']:
                for seed in range(1, 6):
                    out_prefix = "lr_" + lr +  "_maxtok_" + maxtok + \
                            "_lay_" + lay + "_embed_dim_" + embed_dim + \
                            "_kernel_enc_" + kernel_enc + \
                            "_kernel_dec_" + kernel_dec +  "_do_" + do
                    ...
                    train_command = 'python ./train.py ' + args[split] + \
                            ...
                            ' --encoder-layers ' + '\"[('+embed_dim+', '+kernel_enc+')] * '+ lay + '\"' + \
                            ' --decoder-embed-dim ' + embed_dim + \
                            ' --decoder-layers ' + '\"[('+embed_dim+', '+kernel_dec+')] * '+ lay + '\"' + \
    ...
    with open(os.path.join(args['output_folder'], 'kernel_exp_template_train_commands.txt'), 'w') as g:
        for elem in template_train_commands:
            g.write(elem)
            g.write('\n')

    with open(os.path.join(args['output_folder'], 'kernel_exp_random_train_commands.txt'), 'w') as k:
        for elem in random_train_commands:
            k.write(elem)
            k.write('\n')

    with open(os.path.join(args['output_folder'], 'kernel_exp_jump_train_commands.txt'), 'w') as l:
        for elem in jump_train_commands:
            l.write(elem)
            l.write('\n')

    with open(os.path.join(args['output_folder'], 'kernel_exp_template_generate_commands.txt'), 'w') as p:
        for elem in template_generate_commands:
            p.write(elem)
            p.write('\n')

    with open(os.path.join(args['output_folder'], 'kernel_exp_random_generate_commands.txt'), 'w') as q:
        for elem in random_generate_commands:
            q.write(elem)
            q.write('\n')

    with open(os.path.join(args['output_folder'], 'kernel_exp_jump_generate_commands.txt'), 'w') as z:
